...
import { QUAKE_PALETTE, archiveName, convertRgbaToMip, planQuakeResize, reserveTextureName, textureNameFromFilename, validateTextureDimensions } from '../../resources/js/lib/quakeMip.js';
...
const resize=planQuakeResize(1000,700);
assert.equal(resize.targetWidth,1008);
assert.equal(resize.targetHeight,704);
assert.ok(Math.abs(resize.drawWidth/resize.drawHeight-1000/700)<0.000001);
assert.equal(planQuakeResize(1024,768).resized,false);
