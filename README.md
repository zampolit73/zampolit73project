...
| `/projects/cio-presentations` | авторизованный | каталог презентаций; управление источниками и сканированием доступно только admin |
...
Модель доступа: guest видит только главную `/` и `/login`; `user` получает `/projects` и все проекты; `admin` получает тот же доступ плюс административные страницы и управляющие действия.
